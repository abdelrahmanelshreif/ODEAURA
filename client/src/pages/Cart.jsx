...
//           <span className="w-full flex flex-row justify-between">
//             <p>Total</p>
//             <p>EGP {(CartProducts?.subTotal + 21)?.toFixed(2)}</p>
//           </span>
//         </div>
...
const fetchWithAuth = async (url, options = {}) => {
  const token = document.cookie.match(new RegExp('(^| )jwt=([^;]+)'));
  if (token) {
    options.headers = {
      ...options.headers,
...
const Cart = () => {
  ...
  const confimOrderHandler = async (address) => {
    ...
    try {
      ...
      const response = await fetchWithAuth('cart/create-checkout-session', {
        method: 'POST',
        headers,
  ...
  return (
    <>
      <Modal size="md" opened={opened} onClose={close} title="Confirm order">
        <div className="flex flex-col gap-3 w-full justify-between mb-4">
          <span className="w-full flex flex-row justify-between">
            <p>Subtotal</p>
            <p>EGP {CartProducts?.subTotal?.toFixed(2)}</p>
